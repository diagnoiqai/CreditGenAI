      ...
      return (
        <div className="mt-2 space-y-2">
          {userApplications.map((app, i) => (
            <div key={`app-status-${app.id}-${i}`} className="bg-white p-4 rounded-3xl border-2 border-[#E4EAF4] shadow-sm">
              <div className="flex justify-between items-start mb-2">
                <div>
                  <h4 className="font-display font-bold text-[#0D1626]">{app.bankName}</h4>
                  <p className="text-[10px] text-[#8E9BB8] uppercase tracking-wider font-bold">{app.loanType}</p>
                </div>
                <span className={`px-2 py-0.5 rounded-lg text-[10px] font-bold uppercase tracking-wider ${
                  app.status === 'Approved' ? 'bg-green-100 text-green-700' :
                  app.status === 'Rejected' ? 'bg-red-100 text-red-700' :
                  app.status === 'Documents Received' ? 'bg-blue-100 text-blue-700' :
              ...
              {app.subStatus && (
                <div className="mb-2 p-2 bg-[#F2F5FB] rounded-xl border border-[#E4EAF4]">
                  <p className="text-[10px] font-bold text-[#4A5878] uppercase tracking-wider mb-1">Latest Update</p>
                  <p className="text-xs font-bold text-[#1B6EF3]">{app.subStatus}</p>
                </div>
              )}
              {app.statusNotes && <div className="text-xs text-[#4A5878] italic border-l-2 border-[#1B6EF3]/30 pl-2 py-1">"{app.statusNotes}"</div>}
              {app.attachments && app.attachments.length > 0 && (
                <div className="mt-3 pt-3 border-t border-[#E4EAF4]">
                  <p className="text-[10px] font-bold text-[#4A5878] uppercase tracking-wider mb-2">Submitted Documents</p>
                  <div className="flex flex-wrap gap-2">
                    {app.attachments.map((att, idx) => (
                      <a key={`att-${att.fileUrl}-${idx}`} href={att.fileUrl} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 p-2 bg-[#F7F9FD] rounded-xl border border-[#E4EAF4] hover:bg-[#EBF2FF] transition-colors">
                        <FileText className="w-3 h-3 text-[#1B6EF3]" />
                        <span className="text-[10px] font-bold text-[#1253C4] truncate max-w-[100px]">{att.fileName}</span>
                      </a>
                    ))}
                  </div>
                </div>
              ...
              {app.status === 'Rejected' && app.rejectionReason && (
                <div className="mt-2">
                  <button onClick={() => onAskAI(`Why was my ${app.bankName} application rejected?`)} className="text-[10px] font-bold text-[#DC2626] uppercase tracking-wider bg-red-50 px-3 py-1.5 rounded-xl border border-red-100 hover:bg-red-100 transition-colors">
                    Ask AI for Rejection Reason
                  </button>
                </div>
              )}
              <div className="mt-3 pt-3 border-t border-[#E4EAF4] flex justify-between items-center">
                <span className="text-[10px] text-[#8E9BB8] font-medium">Applied on {new Date(app.timestamp).toLocaleDateString()}</span>
                <span className="text-xs font-display font-bold text-[#0D1626]">₹{app.loanAmount.toLocaleString('en-IN')}</span>
              </div>
            </div>
          ))}
        </div>
