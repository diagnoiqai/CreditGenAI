...
export const useChat = (
  ...
  const isGuest = profile.authMethod === 'guest';
  ...
  useEffect(() => {
    ...
    if (messages.length > 0) return; // Already have active session messages
    ...
    const fetchChat = async () => {
      try {
        ...
        if (isGuest) {
          // Check local storage for guest first
          const guestSession = guestSessionService.getGuestSession();
          if (guestSession && guestSession.chatMessages && guestSession.chatMessages.length > 0) {
            history = guestSession.chatMessages;
          }
        }
        
        // If no local history or not a guest, check database
        if (history.length === 0) {
          const dbHistory = await apiService.getChatHistory(profile.uid);
        ...
        if (history && history.length > 0) {
          ...
          try {
            const initialPrompt: ChatMessage = {
              role: 'user',
              content: 'Check my loan eligibility and show me the top 3 best bank offers for me.',
            ...
            if (aiResponse.action && onAIAction) {
              onAIAction(aiResponse.action.data);
            }
            ...
            // Initial save
            if (isGuest) {
              guestSessionService.addChatMessage(assistantMsg);
            }
    ...
    fetchChat();
  }, [profile.uid, profile.displayName, profile.monthlyIncome, bankOffers.length, isGuest]);

  const saveChat = useCallback(async (newMessages: ChatMessage[]) => {
    if (!profile.uid) return;
    
    // Save to guest session (localStorage)
    if (isGuest) {
      const latestMsg = newMessages[newMessages.length - 1];
      if (latestMsg) {
        guestSessionService.addChatMessage(latestMsg);
      }
    }
    // Always save to DB as backup/sync
