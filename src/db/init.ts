...
export const initDb = async () => {
  ...
  try {
    ...
    try {
      console.log('SUCCESS: Connected to PostgreSQL database. Checking schema...');
      
      // SCHEMA ENSURANCE: Ensure dev schema exists
      await client.query('CREATE SCHEMA IF NOT EXISTS dev');
      
      console.log('INFO: Initializing/Updating database schema...');
      ...
      // Core users table (Identity & Loan Requirements)
      await client.query(`
      ...
      await client.query('CREATE INDEX IF NOT EXISTS idx_users_email ON dev.users(email)');
      await client.query('CREATE INDEX IF NOT EXISTS idx_users_role ON dev.users(role)');

      // Extension: Salaried Details
      await client.query(`
        CREATE TABLE IF NOT EXISTS dev.user_salaried_details (
          uid TEXT PRIMARY KEY REFERENCES dev.users(uid) ON DELETE CASCADE,
          company_type TEXT,
          company_name TEXT,
      ...
      // Extension: Self-Employed Details
      await client.query(`
        CREATE TABLE IF NOT EXISTS dev.user_self_employed_details (
          uid TEXT PRIMARY KEY REFERENCES dev.users(uid) ON DELETE CASCADE,
          itr_filed BOOLEAN,
          annual_income_itr NUMERIC,
          annual_income_ideal NUMERIC,
          years_of_business TEXT,
          updated_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP
        )
      `);
      ...
      try {
        ...
        // Phase 1 Auth System Columns
        if (!existingUserCols.includes('phone')) {
          await client.query('ALTER TABLE dev.users ADD COLUMN phone TEXT');
        }
        // Ensure unique constraint on phone column for ON CONFLICT logic
        try {
          // 1. Clean up any potential duplicates before creating the unique constraint
          await client.query(`
            DELETE FROM dev.users a USING dev.users b
            WHERE a.ctid < b.ctid AND a.phone = b.phone AND a.phone IS NOT NULL;
          `);
          
          // 2. Drop old indexes that might conflict or be redundant
          await client.query('DROP INDEX IF EXISTS dev.idx_users_phone');
          await client.query('DROP INDEX IF EXISTS dev.idx_users_phone_unique');
          
          // 3. Ensure the formal constraint exists
          const constraintCheck = await client.query(`
            SELECT constraint_name 
            FROM information_schema.table_constraints 
            WHERE table_schema = 'dev' AND table_name = 'users' AND constraint_name = 'users_phone_key'
          `);
          
          if (constraintCheck.rows.length === 0) {
            // Drop any other existing unique constraints on phone first to avoid duplicates
            await client.query('ALTER TABLE dev.users DROP CONSTRAINT IF EXISTS users_phone_unique');
            await client.query('ALTER TABLE dev.users ADD CONSTRAINT users_phone_key UNIQUE (phone)');
          }
        } catch (e: any) {
          console.warn('WARNING: Could not ensure unique constraint on phone.', e.message);
        }

        if (!existingUserCols.includes('auth_method')) {
          await client.query("ALTER TABLE dev.users ADD COLUMN auth_method TEXT DEFAULT 'guest'");
          // Set existing users to 'phone_otp' as a fallback migration path
          await client.query("UPDATE dev.users SET auth_method = 'phone_otp' WHERE auth_method IS NULL AND email IS NOT NULL");
        }
        if (!existingUserCols.includes('user_type')) {
          await client.query("ALTER TABLE dev.users ADD COLUMN user_type TEXT DEFAULT 'guest'");
        }
        if (!existingUserCols.includes('form_completed')) {
          await client.query('ALTER TABLE dev.users ADD COLUMN form_completed BOOLEAN DEFAULT false');
          // Assume users with employment type defined have completed the form
          await client.query("UPDATE dev.users SET form_completed = true WHERE employment_type IS NOT NULL");
        }
        if (!existingUserCols.includes('form_completed_at')) {
          await client.query('ALTER TABLE dev.users ADD COLUMN form_completed_at TIMESTAMP WITH TIME ZONE');
        }
        if (!existingUserCols.includes('is_new_user')) {
          await client.query('ALTER TABLE dev.users ADD COLUMN is_new_user BOOLEAN DEFAULT true');
          await client.query('UPDATE dev.users SET is_new_user = false WHERE created_at < NOW() - INTERVAL \'1 day\'');
        }
        if (!existingUserCols.includes('otp_token')) {
          await client.query('ALTER TABLE dev.users ADD COLUMN otp_token TEXT');
        }
        if (!existingUserCols.includes('otp_created_at')) {
          await client.query('ALTER TABLE dev.users ADD COLUMN otp_created_at TIMESTAMP WITH TIME ZONE');
        }
        if (!existingUserCols.includes('otp_verified_at')) {
          await client.query('ALTER TABLE dev.users ADD COLUMN otp_verified_at TIMESTAMP WITH TIME ZONE');
        }
      } catch (e: unknown) {
        const errMsg = e instanceof Error ? e.message : String(e);
      ...
      // SEED CLEANUP: Remove old min_income seed column
      ...
      if (parseInt(offersCount.rows[0].count) === 0) {
        ...
        const seedOffers = [
          ['HDFC Bank', 'Personal Loan', 100000, 5000000, 10.5, 15.0, 1.0, 12, 60, 750, 12.5, 80, 'Rahul Sharma', '9876543210', 'rahul@hdfc.com', 'Flexible repayment options available with part-payment facility.', '2% of outstanding principal after 12 months.', 'Standard bank T&C apply. Minimum 2 years work experience required.', '2% of principal amount.'],
          ['ICICI Bank', 'Personal Loan', 50000, 3000000, 10.75, 16.0, 1.5, 12, 72, 720, 13.0, 75, 'Priya Gupta', '9876543211', 'priya@icici.com', 'Monthly EMI through ECS/NACH.', '3% preclosure charges apply.', 'Processing fee is non-refundable.', '3% of outstanding amount.'],
          ['SBI', 'Personal Loan', 25000, 2000000, 9.6, 12.0, 0.5, 12, 60, 700, 11.0, 90, 'Amit Kumar', '9876543212', 'amit@sbi.com', 'No hidden charges. Simple repayment.', 'Nil preclosure charges for government employees.', 'Lowest interest rates for salary account holders.', 'Nil for salary account holders, 1% for others.'],
          ['Axis Bank', 'Personal Loan', 50000, 4000000, 10.49, 14.5, 1.0, 12, 60, 750, 12.0, 85, 'Sanjay Singh', '9876543213', 'sanjay@axis.com', 'Multiple repayment channels.', 'Lock-in period of 6 months.', 'Instant approval for pre-qualified customers.', '5% of outstanding principal.'],
          ['Bajaj Finserv', 'Personal Loan', 30000, 2500000, 11.0, 18.0, 2.0, 12, 84, 750, 14.0, 70, 'Neha Verma', '9876543214', 'neha@bajaj.com', 'Flexi-loan options available.', 'Part-payment allowed at zero cost.', 'Minimal documentation required.', '3% of outstanding principal amount.']
        ];
      ...
      try {
        ...
        await client.query('CREATE INDEX IF NOT EXISTS idx_applications_status ON dev.applications(status)');
      } catch (e: unknown) {
        const errMsg = e instanceof Error ? e.message : String(e);
      ...
      // ============ FINAL CLEANUP: Dropping Unused Columns ============
      try {
        console.log('INFO: Performing schema cleanup...');
        
        // 1. Unify users table mobile -> phone
        const usersTableCheck = await client.query(`
          SELECT column_name 
          FROM information_schema.columns 
          WHERE table_schema = 'dev' AND table_name = 'users'
        `);
        const currentUsersCols = usersTableCheck.rows.map((r: any) => r.column_name);
        
        if (currentUsersCols.includes('mobile') && currentUsersCols.includes('phone')) {
          console.log('INFO: Migrating users mobile to phone if empty...');
          await client.query('UPDATE dev.users SET phone = mobile WHERE (phone IS NULL OR phone = \'\') AND mobile IS NOT NULL');
          await client.query('ALTER TABLE dev.users DROP COLUMN mobile');
          console.log('SUCCESS: Dropped users.mobile');
        }

        // 2. Drop unused bank_offers.min_income
        const bankTableCheck = await client.query(`
          SELECT column_name 
          FROM information_schema.columns 
          WHERE table_schema = 'dev' AND table_name = 'bank_offers'
        `);
        const currentBankCols = bankTableCheck.rows.map((r: any) => r.column_name);
        
        if (currentBankCols.includes('min_income')) {
          await client.query('ALTER TABLE dev.bank_offers DROP COLUMN min_income');
          console.log('SUCCESS: Dropped bank_offers.min_income');
        }

        // 3. Rename applications.user_mobile to user_phone
        const appsTableCheck = await client.query(`
          SELECT column_name 
          FROM information_schema.columns 
          WHERE table_schema = 'dev' AND table_name = 'applications'
        `);
        const currentAppsCols = appsTableCheck.rows.map((r: any) => r.column_name);

        if (currentAppsCols.includes('user_mobile') && !currentAppsCols.includes('user_phone')) {
          await client.query('ALTER TABLE dev.applications RENAME COLUMN user_mobile TO user_phone');
          console.log('SUCCESS: Renamed applications.user_mobile to user_phone');
        }
      } catch (cleanupErr: any) {
        console.warn('WARNING: Schema cleanup encountered errors:', cleanupErr.message);
      }
