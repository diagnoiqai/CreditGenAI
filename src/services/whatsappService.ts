...
export const sendWhatsAppMessage = async (to: string, body: string) => {
  // Explicitly disabled for now as per user request
  console.log(`[WhatsApp Service] Skipping message to ${to} (feature disabled)`);
  return null;

  if (twilioClient && process.env.TWILIO_ACCOUNT_SID && process.env.TWILIO_AUTH_TOKEN && process.env.TWILIO_WHATSAPP_NUMBER) {
    try {
