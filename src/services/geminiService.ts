...
const getGeminiApiKey = () => {
  // Try import.meta.env (Vite standard for client-side variables)
  ...
  if (typeof process !== 'undefined' && process.env?.VITE_GEMINI_API_KEY) {
    console.log('Gemini Service: Using API key from process.env.VITE_GEMINI_API_KEY');
    return process.env.VITE_GEMINI_API_KEY;
  }
  console.warn('Gemini Service: No API key found in import.meta.env or process.env');
  return '';
};
...
- If the user has a REJECTED application, and asks about their status, ALWAYS explain the rejection reason in your text response and include "Why was my application rejected?" in suggestions.
...
`;
  ...
  try {
    ...
    const key = getGeminiApiKey();
    if (!key) {
      return { text: "I'm sorry, but the AI service is not configured. Please ensure your .env file has VITE_GEMINI_API_KEY and you have restarted your dev server." };
    }
