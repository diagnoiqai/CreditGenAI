...
- CRITICAL: NEVER generate markdown tables, comparison grids, or structured checklists in your "text" response. Keep the response strictly conversational and textual.
- CRITICAL: When the user asks for "best offers" or starts the chat for the first time, you MUST focus on the TOP 3 BANKS from the "Offers" list provided. Mention their names and key reasons why they are a good match.
- CRITICAL: The "Offers" list is already sorted by a robust scoring engine. The first bank is the mathematically "Best Match".
- CRITICAL: ALWAYS mention the "Best Match" bank if it satisfies the requirement.
- CRITICAL: Use the match reasons provided to explain why it's a good fit.
...
`;
  ...
  try {
    ...
    // 🎯 OPTIMIZED: Use standard stable model
    ...
    // Robust JSON Extraction for cases where Gemini wraps JSON in markdown or adds text
    ...
    try {
      ...
      console.warn('⚠️ GEMINI: Standard JSON parse failed, attempting extraction...', e);
      ...
      if (jsonMatch) {
        try {
          ...
          throw new Error("Failed to parse AI response into valid JSON even after extraction.");
        }
      } else {
    ...
    if (error?.message?.includes('PERMISSION_DENIED') || error?.status === 403) {
      return { 
        text: "I'm sorry, I don't have permission to use this AI model right now. Please check your Gemini API key settings.", 
        timeTaken: { totalMs: Math.round(errorTime) } 
      };
    }
    ...
    if (!key) {
      return { 
        text: "AI service not configured. Please add GEMINI_API_KEY to your .env file.", 
        timeTaken: { totalMs: Math.round(errorTime) } 
      };
    }
    
    return { 
      text: "Failed to call the Gemini API. Please try again.", 
      timeTaken: { totalMs: Math.round(errorTime) } 
    };
  }
}
