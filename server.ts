...
const initDb = async () => {
  ...
  try {
    ...
    try {
      ...
      // 4. Ensure application_history table has all required columns
      try {
        ...
        `);
        const existingHistCols = histCols.rows.map(r => r.column_name);
    ...
    try {
      ...
      if (twilioClient && process.env.TWILIO_WHATSAPP_NUMBER && profile.mobile) {
        try {
          ...
          console.log(`WHATSAPP SENT TO: ${formattedMobile}`);
        } catch (waError) {
...
app.patch('/api/admin/leads/:id/status', async (req, res) => {
  ...
  try {
    ...
    if (leadId.startsWith('user-')) {
      ...
      const insertResult = await pool.query(
        `INSERT INTO dev.applications 
         (uid, user_name, user_email, user_mobile, loan_type, loan_amount, status, sub_status, status_notes, rejection_reason) 
    ...
    // Record history
    await pool.query(
      'INSERT INTO dev.application_history (application_id, status, sub_status, status_notes, rejection_reason, updated_by) VALUES ($1::integer, $2, $3, $4, $5, $6)',
...
app.get('/api/admin/leads/:id/history', async (req, res) => {
  ...
  try {
    ...
    const applicationId = parseInt(leadId);
    ...
    const result = await pool.query(
      `SELECT h.*, u.display_name as staff_name 
       ...
       WHERE h.application_id = $1::integer 
